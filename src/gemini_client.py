"""Gemini client to see humor judgments, poem continuation, and author knowledge"""
...
def get_gemini_response(gclient, model, prompt, cf, call_type):
  ...
  Args:
      gclient (openai.OpenAI): The OpenAI client.
  ...
  resps = [cand.content.parts[0].text for cand in completion.candidates]
...
def process_gemini_response(gclient, model, cf, fn, poem_text, call_type, suffix=None):
  """
  ...
  Args:
      gclient (genai.Client): The Gemini client.
      model (str): The model to use for generating the response.
      cf (module): The configuration module.
      fn (str): The filename of the poem.
      poem_text (str): The text of the poem.
  ...
  comp, resps, resp_time = get_gemini_response(
    gclient, model, prompt, cf, call_type)
...
def parse_args():
  ...
  """
  parser = argparse.ArgumentParser(description="Gemini client.")
...
if __name__ == "__main__":
  ...
  active_models = cf.gemini_models
  ...
  resp_times = {"poem_id": [],
                ...
                "gpt-4o": [],
                "gpt-4o-mini": [],
                "deepseek-chat": [], "deepseek-reasoner": [], "gemini-flash": [],
                "call_type": []}
    ...
    if reuse_df:
      ...
      resp_time_df = resp_time_df.astype(
        {"poem_id": "int64", "gpt-3.5-turbo": "float64", "gpt-4": "float64",
         ...
         "gpt-4o-mini": "float64", "deepseek-chat": "float64", "deepseek-reasoner": "float64", "gemini-flash": "float64",
         "call_type": "category"})
    ...
    for fn in sorted(os.listdir(cf.corpus_dir)):
      ...
      for call_type in cf.call_types:
        ...
        if int(completion_suffix) >= 2 and call_type != "humor":
          continue
        resp_time_df = process_gemini_response(
          gemini_client, model, cf, fn, poem_text, call_type, suffix=completion_suffix)
